...
if (sys.argv[1] == "1"):
    ...
    plotLabel = "Timing Optimized, Low Threshold"
...
if (sys.argv[1] == "2"):
    ...
    plotLabel = "Timing Optimized, High Threshold"
...
if (sys.argv[1] == "3"):
    ...
    plotLabel = "Timing Not Optimized, Low Threshold"
...
if (sys.argv[1] == "4"):
    ...
    plotLabel = "Timing Not Optimized, High Threshold"
...
###############################################
#Time resolution for full dataset
###############################################
deltaTHist = TH1F("deltaTHist",";#Delta t [ns];Number of Eventts", 100, -10, 10)
tree.Draw(deltaTDrawCommand + " >> deltaTHist","ntracks==1 && y_dut[0] > 0 && npix>0 && nback>0 " + yPositionSelection + xPositionSelection + selectionCommand,"colz")

fitresult = deltaTHist.Fit("gaus","S", "", -1.5, 1.5)
sigma = round(fitresult.Parameter(2),2)
sigmaErr = round(fitresult.ParError(2),2)

print "sigma : " + str(sigma) + " +/- " + str(sigmaErr)

deltaTHist.SetStats(False)
deltaTHist.GetYaxis().SetTitleOffset(1.2)
deltaTHist.GetXaxis().SetRangeUser(-4,6)
deltaTHist.SetMaximum( 1.2*deltaTHist.GetMaximum())

title = TLatex()
title.SetTextSize(0.05);
#title.SetTextAlign(13);  
title.DrawLatexNDC(.2,.93,"CACTUS Pixel "+pixelName+" Digital");
title.SetTextSize(0.035);
title.DrawLatexNDC(.2,.86, plotLabel);
title.SetTextSize(0.03);
title.DrawLatexNDC(.6,.6,"Time Resolution:");
title.DrawLatexNDC(.6,.55,str(sigma) +" +/- " + str(sigmaErr) + "ns");
c.Update()
c.SaveAs("CACTUS_TimeResolution_" + outputLabel + ".gif")
...
for b in range(1,tmpHist2D.GetXaxis().GetNbins()+1):
    ...
    if (projectionHist.Integral() > 20) :         
        fitresult = projectionHist.Fit("gaus","S", "", -1.5, 1.5)
        tmpSigma = round(fitresult.Parameter(2),2)
        tmpSigmaErr = round(fitresult.ParError(2),2)        
 
    sigma.append(tmpSigma)
    sigmaErr.append(tmpSigmaErr)


xArr = array.array('f',x)
xErrArr = array.array('f',xErr)
yArr = array.array('f',sigma)
yErrArr = array.array('f',sigmaErr)
...
effGraph.GetYaxis().SetTitle("Time resolution [ns]")
...
c.Update()
c.SaveAs("CACTUS_TimeResolutionVsX" + outputLabel + ".gif")
...
title.DrawLatexNDC(.2,.93,"CACTUS Pixel "+pixelName+" Analog");
c.Update()
c.SaveAs("CACTUS_TimeResolutionVsY" + outputLabel + ".gif")
